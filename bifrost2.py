...
if platform.system() == "Darwin":  # macOS
    BASE_DIR = Path.home() / "Documents" / "Bifrost_2"
...
class BifrostApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Bifrost 2 - Text Extraction Suite")
    ...
    def setup_gui(self):
        ...
        ttk.Label(header_frame, 
                text="Bifrost 2 - Text Extraction Suite",
        ...
        self.canvas.bind("<ButtonPress-1>", self.start_selection_or_pan)
        self.canvas.bind("<B1-Motion>", self.update_selection_or_pan)
        self.canvas.bind("<ButtonRelease-1>", self.finalize_selection_or_pan)
        self.root.bind("<KeyPress-s>", lambda e: self.toggle_selection_mode())
    ...
    def start_selection_or_pan(self, event):
        if self.selection_mode:
            self.start_selection(event)
        else:
            self.start_pan(event)

    def update_selection_or_pan(self, event):
        if self.selection_mode:
            self.update_selection(event)
        else:
            self.pan_image(event)

    def finalize_selection_or_pan(self, event):
        if self.selection_mode:
    ...
    def toggle_selection_mode(self):
        ...
        cursor = "cross" if self.selection_mode else ""
        self.canvas.config(cursor=cursor)
        self.update_status("Selection mode: ON" if self.selection_mode else "Selection mode: OFF")
    ...
    def start_selection(self, event):
        if self.original_image:
            ...
            self.preview_rect = self.canvas.create_rectangle(
                *self.select_start, *self.select_start,
                outline="#00FF00", width=2, tags="preview_sel"
            )
    ...
    def update_selection(self, event):
        if self.select_start:
            x = self.canvas.canvasx(event.x)
            y = self.canvas.canvasy(event.y)
            self.canvas.coords(self.preview_rect, *self.select_start, x, y)

    def finalize_selection(self, event):
        if self.select_start and self.original_image:
            try:
                ...
                self.process_selected_region(img_coords)
            except Exception as e:
    ...
    def canvas_to_image_coords(self, x1, y1, x2, y2):
        img_x1 = int((min(x1, x2) - self.image_position[0]) / self.image_scale)
        img_y1 = int((min(y1, y2) - self.image_position[1]) / self.image_scale)
        img_x2 = int((max(x1, x2) - self.image_position[0]) / self.image_scale)
        img_y2 = int((max(y1, y2) - self.image_position[1]) / self.image_scale)

        img_w, img_h = self.original_image.size
        return (
            max(0, min(img_x1, img_w)),
            max(0, min(img_y1, img_h)),
            max(0, min(img_x2, img_w)),
            max(0, min(img_y2, img_h))
        )

    def process_selected_region(self, coordinates):
        try:
            cropped = self.original_image.crop(coordinates)
            img_array = np.array(cropped.convert('RGB'))
            gray = cv2.cvtColor(img_array, cv2.COLOR_RGB2GRAY)
            processed = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)[1]
            
            text = pytesseract.image_to_string(processed)
            self.txt_display.insert(tk.INSERT, text)
            self.extracted_text += text
            self.auto_save_text()
        except Exception as e:
            self.show_error(f"Region extraction failed: {str(e)}")
